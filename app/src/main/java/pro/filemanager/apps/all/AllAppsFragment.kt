...
class AllAppsFragment : BaseFragment(), Observer<MutableList<AppItem>> {
    lateinit var viewModel: AllAppsViewModel
        ...
        if(viewModel.selectionTool.selectedPaths.isNotEmpty()) {
            InfoTool.showInfoForLibraryItems(
                ...
                mutableListOf<BaseLibraryItem>().apply {
                    (binding.fragmentImageLibraryList.adapter as AllAppsAdapter).currentList.forEach {
    ...
    override fun onChanged(newItems: MutableList<AppItem>?) {
        if(isViewModelInitialized() && isBindingInitialized() && binding.fragmentImageLibraryList.adapter != null) {
            ...
            refreshListAdapter(binding.fragmentImageLibraryList.adapter as ListAdapter<AppItem, RecyclerView.ViewHolder>, newItems!!, viewModel.shouldScrollToTop)
    ...
    suspend fun initList(imageItems: MutableList<AppItem>) {
        ...
        binding.fragmentImageLibraryList.adapter = AllAppsAdapter(requireActivity(), imageItems, layoutInflater, this).apply {
    ...
    override fun launchCore() {
        PermissionWrapper.requestStorageGroup(
            ...
            CoroutineWrapper.globalMainScope.launch {
                viewModel = ViewModelProvider(this@AllAppsFragment, AllAppsViewModelFactory(frContext, ImageRepo.getSingleton(), folderItem)).get(AllAppsViewModel::class.java)

                viewModel.getItemsLive(frContext).observe(viewLifecycleOwner, this@AllAppsFragment)
