...
class AllAppsViewModel(val context: @RawValue Context, var imageRepo: @RawValue ImageRepo, var folderItem: ImageFolderItem?) : BaseViewModel() {

    private var itemsLive: MutableLiveData<MutableList<AppItem>>? = null
        ...
        if(itemsLive != null) {
            when(currentSortOrder) {
                SortTool.SORT_ORDER_DATE_RECENT -> {
                    search(
                            if(folderItem != null) {
                                val imageItems = mutableListOf<AppItem>()
    ...
    private suspend fun search(items: MutableList<AppItem>) {
    ...
    suspend fun getItemsLive(context: Context): LiveData<MutableList<AppItem>> {
